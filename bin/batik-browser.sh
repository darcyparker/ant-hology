...
#See http://xmlgraphics.apache.org/batik/tools/browser.html
...
if [ ! -d "$_batikDir" ] || [ ! -s "$_batikDir/batik-squiggle-1.8.jar" ]; then
  echo "Error: Could not find \"$_batikDir/batik-squiggle-1.8.jar\"" > /dev/stderr
  return
fi
...
#Execute batik-squiggle (the SVG Browser) using its command line intereface.
#To do: Replace jar with main class and setup classpath
java \
  -jar "$_batikDir/batik-squiggle-1.8.jar" \
  "$@"
