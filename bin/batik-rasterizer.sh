...
if [ ! -d "$_batikDir" ] || [ ! -s "$_batikDir/batik-rasterizer.jar" ]; then
  echo "Error: Could not find \"$_batikDir/batik-rasterizer.jar\"" > /dev/stderr
  return
fi
...
java \
  -jar "$_batikDir/batik-rasterizer.jar" \
  "$@"
