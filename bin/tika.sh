...
_tikaDir="$_libDir/apache-tika-app-1.10"
...
if [ ! -d "$_tikaDir" ] || [ ! -s "$_tikaDir/tika-app-1.10.jar" ]; then
  echo "Error: Could not find \"$_tikaDir/tika-app-1.10.jar\"" > /dev/stderr
  return
fi
...
if [ -z "$*" ]; then
  java \
    -jar "$_tikaDir/tika-app-1.10.jar" \
    -?
else
  java \
