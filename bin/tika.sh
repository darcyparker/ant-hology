...
_tikaDir="$_libDir/apache-tika-app-1-4"
...
if [ ! -d "$_tikaDir" ] || [ ! -s "$_tikaDir/tika-app-1.4.jar" ]; then
  echo "Error: Could not find \"$_tikaDir/tika-app-1.4.jar\"" > /dev/stderr
  return
fi
