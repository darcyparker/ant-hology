...
<iso:schema
  ...
  >
  <!--Example based on http://www.dpawson.co.uk/schematron/keys.html#keys.ex2 -->
  ...
  <iso:pattern id="name.checks">
    ...
    <iso:rule context="chapter[@xml:id]">
      <iso:assert test="not(key('href', concat(@xml:id, '.html')))">Warning.  if chapter id value is used as html filename, there will be a conflict with <iso:value-of select="@xml:id"/>.html</iso:assert>
    </iso:rule>
  </iso:pattern>
</iso:schema>
